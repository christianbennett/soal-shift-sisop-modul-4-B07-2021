...
void logFile(char *nama, char *fpath){
	time_t rawtime;
	struct tm *timeinfo;
	time(&rawtime);
...
void logFile2(char *nama, const char *from, const char *to){
	time_t rawtime;
	struct tm *timeinfo;
	time(&rawtime);
	timeinfo = localtime(&rawtime);
...
void encryptAtbash(char *path){
	...
	if(endid == strlen(path)) endid = dotId(path);
...
static int xmp_rename(const char *from, const char *to){
	...
	logFile2("RENAME", frompath, topath);
	
	if (c != NULL){
		enkripsi2(topath);
		logFile2("ENCRYPT2", from, to);
	}

	if (b != NULL && c == NULL){
		dekripsi2(topath);
		logFile2("DECRYPT2", from, to);
	}
	
	if (strstr(to, aisa) != NULL){
		encryptBinary(topath);
		logFile2("ENCRYPT3", from, to);
	}
	
	if (strstr(from, aisa) != NULL && strstr(to, aisa) == NULL){
		decryptBinary(topath);
		logFile2("DECRYPT3", from, to);
	}
...
static int xmp_symlink(const char *from, const char *to)
{
	...
	logFile2("SYMLINK", frompath, topath);
